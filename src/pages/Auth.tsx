...
import { sendLoginEmail, sendWelcomeEmail } from "@/services/emailService";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        const emailResult = await sendLoginEmail(email, name || email.split('@')[0]);
        if (!emailResult.success) {
          console.warn('Login email notification failed:', emailResult.error);
        }
        ...
        if (!emailResult.success) {
          console.warn('Welcome email notification failed:', emailResult.error);
        }
            ...
            <p className="text-muted-foreground mt-2">
              {isForgotPassword
                ...
                : isLogin
                  ...
                  : "Create your account and start fishing"}
            </p>
          </div>
